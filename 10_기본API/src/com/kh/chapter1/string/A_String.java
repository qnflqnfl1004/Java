...
//		System.out.println(str1.toString());
	...
	public void method2() {
		...
		//    문자열의 beginIndex 위치에서부터 끝까지의 새로운 문자열을 생성해서 리턴한다.
		...
		// 4. indexOf(String str) : int => 전달받은 str의 시작 인덱스를 리턴한다. (주어진 문자열이 포함되어 있지 않으면 -1 리턴)
		//
		//    indexOf(int ch) : int => 전달받은 ch의 시작 인덱스를 리턴한다. (주어진 문자가 포함되어 있지 않으면 -1 리턴)
...
//		index = str.indexOf('o');
		...
		// 5. replace(char oldChar char newChar) : String => 문자열에서 old 문자를 new문자로 변경된 새로운 문자열을 생성해서 리턴한다.
		...
		// 6. toUpperCase(), toLowerCase => 문자열을 모두 대/소문자로 변경한 새로운 문자열을 생성해서 리턴한다.
	...
	public void method3() {
		// 구분자를 이용하여 문자열 분리
		String str = "Java,Oracle,JDBC,HTML,CSS,Spricg";
...
//		}
		...
		// 2. StnrigTokenizer 객체를 이용하는 방법
		//    countTokens() : 남아있는 토큰의 수를 리턴한다.
		//    hasMoreTokens() : 남아있는 토큰이 있는 지 확인한다.
