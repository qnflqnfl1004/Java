 ...
 * 		- 필드의 생성과 소멸
 * 		  생성 시점 : new 연산자를 통해서 객체(인스턴스) 생성 시 메모리(Heap)에 할당된다.
 ...
 * 		- 지역 변수의 생성과 소멸
 * 		  생성 시점 : 특정한 구역({}) 실행 시 메모리(Stack) 영역에 할당 된다.
 ...
 * 		- 클래스 변수의 생성과 소멸
 * 		  생성 시점 : 프로그램이 실행 될 때 JVM에 의해서 해당 클래스가 로드될 때 메모리(static) 영역에 할당된다.
 * 		  소멸 시점 : 프로그램이 종료 될 때 소멸된다.
 */
...
//		public int local; // 지역 변수에는 접근 제한자를 붙이지 않는다.
		...
		//			미 초기화 시 JVM이 기본값으로 초기화한다. (정수 : 0, 실수 : 0.0, 문자 : '\u0000', 논리값 : false, 참조값 : null)
		...
		// 매개 변수 출력 : 매개 변수는 메소드 호출 시 반드시 값이 전달되어 오기 떄문에 바로 출력이 가능하다.
